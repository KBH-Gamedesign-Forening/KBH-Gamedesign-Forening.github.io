...
<section class="relative min-h-screen overflow-hidden">
	...
	<div>
		<button
			onclick={prev}
			class="absolute top-1/2 left-4 z-20 -translate-y-1/2 rounded-full bg-black/50 p-2 transition-colors hover:bg-black/70"
		>
		...
		<button
			onclick={next}
			class="absolute top-1/2 right-4 z-20 -translate-y-1/2 rounded-full bg-black/50 p-2 transition-colors hover:bg-black/70"
		>
			<img src="/right.svg" alt="Next" class="h-8 w-8" />
		</button>

		<div
			class="absolute bottom-26 left-1/2 z-20 flex -translate-x-1/2 flex-col items-center gap-1 rounded-xl bg-black/50 px-4 py-2 text-center text-white"
		>
			<span class="text-xs tracking-wide uppercase opacity-80">{imageIndex} / {totalImages}</span>
		</div>

		<a
			class="absolute bottom-8 left-1/2 z-20 flex -translate-x-1/2 flex-col items-center gap-1 rounded-xl bg-black/50 px-4 py-2 text-center text-white"
			href="#resources"
		>
			<span class="text-sm font-medium">Resurcer</span>
	...
	<div class="relative z-10 flex min-h-screen items-center justify-center">
		<div class="relative">
			<div class="hero-blur absolute inset-0 -m-32 backdrop-blur-md"></div>
...
<style>
	.hero-blur {
		mask-image: radial-gradient(ellipse closest-side at 50% 50%, black 60%, transparent 100%);
